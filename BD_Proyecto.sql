...
create table tbl_Ventas(Id_Ventas number not null,Id_flor number not null, Id_Usuario number not null, Nombre_Cliente varchar2(50)not null, Cantidad number not null, Fecha_Venta varchar2(50) not null,
constraint flor_fk foreign key(Id_flor) references tbl_flores(Id_flor), constraint usuario_fk foreign key(Id_Usuario) references tbl_Usuarios(Id_Usuario), primary key(Id_Ventas));

create table tbl_Pedidos(Id_Pedido number not null, Id_Flor number not null, Cantidad number not null, Fecha_Entrega varchar2(50) not null, 
Fecha_Pedido varchar2(50) not null, constraint flor2_fk foreign key(Id_Flor) references tbl_Flores(Id_Flor), primary key(Id_Pedido));

create table tbl_Inventarios(Id_Inventario number not null, Id_Flor number not null, Vida_Util varchar2(30)not null, Cantidad_Rollos number not null, Fecha_Corte varchar2(50) not null, Estado varchar2(30),
